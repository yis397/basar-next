...
import { Box, Button, Card, CardMedia, Grid, IconButton, Link, Typography} from '@mui/material';
...
interface Props{
  ...
  const ComprasPage:NextPage<Props>=({compras})=> {
      ...
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
        ...
        compras.map((e,t)=>
            <Grid key={e._id} item xs={4}>
              {
                e.items.map((j,i)=>
                  <ComprasCard key={i} item={j} estado={e.estado} comprado={e.creado} idOrden={e._id} tipo={1}/>)
              }
...
export const getServerSideProps: GetServerSideProps = async ({ req }) => {
    ...
    const compras: any = await dbOrden.getCompras(req.cookies.token??"");
